...
(defun coqc/compile (.file-name)
  (cond
    ((equal (shell-command (concat "ls " .file-name)) 0)
     (color-message 93 (concat " (coqc/compile) " .file-name))
     (if (equal (shell-command (concat "coqc " .file-name) nil) 0)
         (color-message 95 "[ok]")
         (color-message 101 "[fail]")))
    (t
     (color-message
      96
      (concat " (coqc/compile) [not find] " .file-name)))
    ))
...
(org/tangle "learning-coq.org")
(coqc/compile "bool.v")
(coqc/compile "nat.v")
(coqc/compile "simpl.v")
(coqc/compile "rewrite.v")
(coqc/compile "destruct.v")
(coqc/compile "induction.v")
(coqc/compile "assert.v")
